...
def load_build_info() -> dict[str, Any]:
    ...
    try:
        raw = resources.files("osr_updater").joinpath("BUILD_INFO.json").read_text(
            encoding="utf-8"
        )
...
def load_third_party_notices() -> str:
    try:
        return resources.files("osr_updater").joinpath(
            "THIRD_PARTY_NOTICES.txt"
        ).read_text(encoding="utf-8")
    except (FileNotFoundError, OSError, UnicodeDecodeError):
        source_notice = Path(__file__).resolve().parents[1] / "THIRD_PARTY_NOTICES.txt"
        try:
            return source_notice.read_text(encoding="utf-8")
        except (FileNotFoundError, OSError, UnicodeDecodeError):
