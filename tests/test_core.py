#!/usr/bin/env python3
"""Focused regression tests for the OSR Updater protocol engine."""
...
def export_lines(items: tuple[core.VehicleConfigItem, ...] | None = None) -> list[str]:
    ...
    return [
        ...
        f"CONFIG_EXPORT_SOURCE: ProjectVer={SOURCE_VERSION}, Profile=test, Schema=1",
        f"CONFIG_EXPORT_TARGET: ProjectVer={TARGET_VERSION}, Profile=test, Schema=1",
...
class CurrentEngineContractTest(unittest.TestCase):
    def test_export_parser_matches_independent_hash_and_accepts_uppercase_blob(self):
        ...
        self.assertEqual(
            core.calculate_vehicle_config_sha256(SOURCE_VERSION, "test", 1, items),
    ...
    def test_private_backup_is_read_back_and_tampering_is_rejected(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(
                document["kind"],
                "osr_updater_vehicle_settings_backup",
            )
