#!/usr/bin/env python3
"""Proven serial, configuration-backup, and App OTA engine for OSR Updater.

This module is UI-independent and does not contain firmware assets or product
catalogue mappings.
"""
...
# list deliberately excludes implementation-specific metadata and unknown keys.
...
class AuditLogger:
    ...
    def __init__(self, directory: Path | None):
        if directory is None:
            ...
            directory = state_root / "osr-updater" / "audit"
...
def _default_snapshot_directory() -> Path:
    ...
    if not state_root.is_absolute():
        state_root = Path.home() / ".local" / "state"
    return state_root / "osr-updater" / "snapshots"
...
def _write_snapshot(snapshot: DeviceSnapshot, directory: Path | None) -> tuple[str, Path]:
    ...
    try:
        ...
        raise AuditError("could not create the private vehicle-settings snapshot") from None
...
def _vehicle_backup_document(
    ...
    return {
        ...
        "kind": "osr_updater_vehicle_settings_backup",
        "captured_at": datetime.now(timezone.utc).replace(microsecond=0).isoformat(),
...
def _load_vehicle_config_backup(path: Path) -> tuple[VehicleConfigExport, str]:
    ...
    try:
        if (
            ...
            or document.get("kind") != "osr_updater_vehicle_settings_backup"
